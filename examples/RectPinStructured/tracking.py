...
if __name__ == "__main__":
    mesh_path = Path(__file__).with_name("RectPin.msh").resolve()
    if not mesh_path.exists():
        raise FileNotFoundError(f"Mesh file not found: {mesh_path}. Run model.py first.")

    start = time.perf_counter()
    mesh = GmshFileTranslator(mesh_path, geom_type="rect").extractDataFromGmshFile()
    ...
    track_path = mesh_path.with_name(f"RectPin-{spacing}-{azim}.trk")
